...
type Presenca = {
  ...
  validado_professor_por: string | null;
  validado_professor_em: string | null;
};

export default function PresencasProfessor() {
  ...
  function textoEstadoProfessor(presenca: Presenca) {
    if (presenca.estado_professor === "validado") return "Validado";
    if (presenca.estado_professor === "rejeitado") return "Rejeitado";

    return "Pendente";
  }

  function presencaValidada(presenca: Presenca) {
    return presenca.estado_professor === "validado";
  }
  ...
  async function carregarDados() {
    ...
    const { data: presencasData, error: presencasError } = await supabase
      ...
      .select(
        ...
        observacoes,
        horas_reposicao,
        validado_professor_por,
        validado_professor_em
      `
      )
  ...
  const totais = useMemo(() => {
    ...
    const totalValidadas = presencas.filter(
      (presenca) => presenca.estado_professor === "validado"
    ).length;
  ...
  async function validarPresenca() {
    if (!professorId || !presencaSelecionada) return;
    ...
    const { error } = await supabase
      .from("presencas")
      .update({
        estado_professor: "validado",
        validado_professor_por: professorId,
    ...
    if (error) {
      console.log("ERRO VALIDAR PRESENÇA:", error);
  ...
  async function validarTudo() {
    ...
    const idsPendentes = presencas
      .filter((presenca) => presenca.estado_professor !== "validado")
    ...
    if (error) {
      console.log("ERRO VALIDAR TUDO:", error);
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        ...
        {loading ? (
          ...
          <>
            ...
            >
              <Ionicons name="checkmark-done-outline" size={22} color="#160909" />
                    ...
                    <View key={presenca.id} style={styles.presencaCard}>
                      ...
                      {validada ? (
                        <Text style={styles.validadoTexto}>
                          Validado em{" "}
                          {formatarData(presenca.validado_professor_em)}
                        </Text>
                      ) : (
                        <Pressable
